...
public class BPMPlayer : MonoBehaviour{
    ...
    private void Start(){
        ...
        if(BMSInfo.min_bpm < BMSInfo.max_bpm){
            ...
            catch(OverflowException){ min_val.text = "∞"; }
            max.SetActive(true);
            try{ max_val.text = (BMSInfo.max_bpm * MainVars.speed).ToString("G29"); }
            catch(OverflowException){ max_val.text = "∞"; }
        }
    }
    private void Update(){
        ...
        if(!BMS_Player.no_bpm_notes){
            while(BMS_Player.bpm_table_row < BMSInfo.bpm_list_table.Count){
                if(BMSInfo.bpm_list_table[BMS_Player.bpm_table_row].time <= BMS_Player.playingTimeAsMilliseconds){
