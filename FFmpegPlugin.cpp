...
// static void* VideoColor32s = NULL;
...
extern "C" float* GetAudioSamples(const char* path, int32_t* channels, int32_t* frequency, int32_t* length){
    ...
    *channels = *frequency = *length = 0;
    ...
    if(!openCodecContext(fc, &audioStreamIdx, AVMEDIA_TYPE_AUDIO, &cc)){
        goto cleanup;
    }
...
/*extern "C" bool AudioIsMidi(const char* path){
    return false;
}*/
// extern "C" void* GetVideoColor32s(){
//     return VideoColor32s;
// }
