...
extern "C" libvlc_media_player_t* PlayerNew(libvlc_media_t* media,
uint32_t width, uint32_t height, uint8_t* data, int64_t ms = 0){
    if(media == NULL || width < 1 || height < 1) return NULL;
    libvlc_media_player_t* player = libvlc_media_player_new_from_media(media);
    if(player != NULL){
        libvlc_video_set_format(player, "RGBA", width, height, width * 4);
        libvlc_video_set_callbacks(player, lock, NULL, NULL, data);
...
// g++ -O3 -shared -Wall -I"./include" -L"./VLC~" -o VLCPlugin.dll VLCPlugin.cpp -lvlc
#else
